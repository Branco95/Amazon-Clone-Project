...
describe("test suite: renderOrderSummary()", () => {
  ...
  beforeAll((done) => {
    loadProducts(() => {
      done(); //this makes sure that the products load first, and then it will run done()
    });
  });
  ...
  it("removes a product", () => {
    document.querySelector(`.js-delete-link-${productId1}`).click(); //after clicking delete we should have just 1 item
    expect(document.querySelectorAll(".js-cart-item-container").length).toEqual(
      1
    );
    expect(
